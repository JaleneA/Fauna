...
// API IMPLEMENTATION - Instagram's Basic Display API //
const accessToken = 'IGQVJWb3llUDlHQWZADYjVxX1hIOGRXVS1VMkZApMVE0LTE2T3gzZADkyNUJ5REdLdEV3R0ZACel9GTXJuWFZAyTXNUZA1VRdHlHbUIzNG5iZAVJhbURhdXlPblU0OE10ZAzEzOTJaYmxFWlRvekZAsMVlZAU2FzYQZDZD';

// Retrieve media data and render feeds on page
fetch(`https://graph.instagram.com/me/media?fields=id,caption,username&access_token=${accessToken}`)
  .then(response => response.json())
  .then(data => {
    // Extract media IDs from response
    const mediaIds = data.data.map(media => media.id);

    // Retrieve media details for each ID and render feed
    mediaIds.forEach((mediaId, index) => {
      const time = Math.floor(Math.random() * 59) + 1;

      fetch(`https://graph.instagram.com/${mediaId}?fields=id,media_type,media_url,thumbnail_url&access_token=${accessToken}`)
        .then(response => response.json())
        .then(media => {
              ...
              <div class="feed">
                <div class="head">
                  <div class="user">
                    ...
                    <div class="info">
                      <h3>${data.data.find(item => item.id === media.id).username}</h3>
                ...
                <div class="caption">
                  <p><b>${data.data.find(item => item.id === media.id).username}</b> ${data.data.find(item => item.id === media.id).caption} <span class="hashtag"> #dogs</span></p>
                </div>
